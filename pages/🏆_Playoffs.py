...
# Set page config
st.set_page_config(page_title="Playoff Stats",
                   page_icon=":basketball:",
                   layout="wide")

# Extract data from tables
table = pd.read_html("https://www.basketball-reference.com/players/c/curryst01.html", attrs={"id": "playoffs_per_game"})
table2 = pd.read_html("https://www.basketball-reference.com/players/c/curryst01.html", attrs={"id": "playoffs_totals"})
...
df = df.drop(index=8, columns=['Tm', 'Lg', 'Pos'])
df2 = df2.drop(index=8, columns=['Tm', 'Lg', 'Pos', 'Unnamed: 30'])

# Set title of page
st.markdown("<h1 style='text-align: center;'>🏀Steph Curry's Playoff Career Statistics🏀</h1>", unsafe_allow_html=True)
...
st.header("Steph Curry's Playoff Per Game Career Stats")
st.dataframe(df)
